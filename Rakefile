...
task :post do
  ...
  abort("rake aborted: must define category") unless category

  path = File.join(SOURCE, category, '_posts')
  abort("rake aborted: '#{category}/_posts' directory not found.") unless FileTest.directory?(path)
  ...
  filename = File.join(path, "#{date}-#{slug}.#{CONFIG['post_ext']}")
